...
c*********************************************************************72
c
cc MAIN is the main program for CONSTANT_TYPE.
c
c  Discussion:
c
c    CONSTANT_TYPE demonstrates that constants may need to be given a type.
c
c    It is natural to assume that if you go to the trouble of typing
c    14 decimals in a constant, that FORTRAN will somehow use them.
c    But constants have a type, just like variables.  If you don't
c    somehow specify a type, then a constant will be treated like
c    a single precision value (assuming you used a decimal point 
c    somewhere.)  
c
c    This means that, for instance, if you print out the value of 
c    ( 1.0203040506070809 - 1.020304050607 ), it will almost certainly 
c    be 0.
c
c  Licensing:
c
