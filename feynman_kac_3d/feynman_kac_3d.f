...
c*********************************************************************72
c
cc MAIN is the main program for FEYNMAN_KAC_3D.
c
c  Discussion:
c
c    This program is derived from section 2.5, exercise 2.2 of Petersen and Arbenz.
c
c    The problem is to determine the solution U(X,Y,Z) of the following 
c    partial differential equation:
c
c      (1/2) Laplacian U - V(X,Y,Z) * U = 0,
c
c    inside the elliptic domain D:
c 
c      D = { (X,Y,Z) | (X/A)^2+(Y/B)^2+(Z/C)^2 <= 1 }
c   
c    with the boundary condition U(boundary(D)) = 1.
c
c    The V(X,Y,Z) is the potential function:
c
c      V = 2 * ( (X/A^2)^2 + (Y/B^2)^2 + (Z/C^2)^2 ) + 1/A^2 + 1/B^2 + 1/C^2.
c
c    The analytic solution of this problem is already known:
c
c      U(X,Y,Z) = exp ( (X/A)^2 + (Y/B)^2 + (Z/C)^2 - 1 ).
c
c    Our method is via the Feynman-Kac Formula.
c
c    The idea is to start from any (x,y,z) in D, and
c    compute (x+Wx(t),y+Wy(t),z+Wz(t)) where 3-D Brownian motion
c    (Wx,Wy,Wz) is updated each step by sqrt(h)*(z1,z2,z3),
c    each z1,z2,z3 are independent approximately Gaussian 
c    random variables with zero mean and variance 1. 
c
c    Each (x1(t),x2(t),x3(t)) is advanced until (x1,x2,x3) exits 
c    the domain D.  
c
c    Upon its first exit from D, the sample path (x1,x2,x3) is stopped and a 
c    new sample path at (x,y,z) is started until N such paths are completed.
c 
c    The Feynman-Kac formula gives the solution here as
c
c      U(X,Y,Z) = (1/N) sum(1 <= I <= N) Y(tau_i),
c
c    where
c
c      Y(tau) = exp( -int(s=0..tau) v(x1(s),x2(s),x3(s)) ds),
c
c    and tau = first exit time for path (x1,x2,x3). 
c
...
c                [ x3(t) + sqrt ( h ) * z3 ]
c
c    Here Z1, Z2, and Z3 are approximately normal univariate Gaussians. 
c
...
c
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) 
     &  '  The calculation takes place inside a 3D ellipsoid.'
      write ( *, '(a)' ) 
     ...
     &  ' trajectories'
      ...
      write ( *, '(a)' ) ' '
      write ( *, '(a)' ) '  The ellipsoid parameters A, B, C are:'
      write ( *, '(a)' ) ' '
      write ( *, '(a,g14.6)' ) '    A = ', a
      write ( *, '(a,g14.6)' ) '    B = ', b
      write ( *, '(a,g14.6)' ) '    C = ', c
...
c
      rth = sqrt ( dble ( dim ) * h )
c
c  Choose the spacing so we have about 10 points in the shortest direction.
c
      if ( a .eq. min ( min ( a, b ), c ) ) then
        ni = 11
        nj = 1 + i4_ceiling ( b / a ) * ( ni - 1 )
        nk = 1 + i4_ceiling ( c / a ) * ( ni - 1 )
      else if ( b .eq. min ( min ( a, b ), c ) ) then
        nj = 11
...
c*********************************************************************72
c
cc POTENTIAL evaluates the potential function V(X,Y,Z).
c
c  Licensing:
c
c    This code is distributed under the GNU LGPL license. 
...
c  Parameters:
c
c    Input, double precision A, B, C, the parameters that define the ellipse.
c
c    Input, double precision X, Y, Z, the coordinates of the point.
c
c    Output, double precision V, the value of the potential function at (X,Y,Z).
c
      implicit none
