#!/bin/bash
#
gfortran -c -g polygon_moments_prb.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling polygon_moments_prb.f"
  exit
fi
rm compiler.txt
#
gfortran polygon_moments_prb.o -L$HOME/libf77/$ARCH -lpolygon_moments
if [ $? -ne 0 ]; then
