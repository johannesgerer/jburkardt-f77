#!/bin/bash
#
gfortran -c -g test_int_laguerre_prb.f >& compiler.txt
if [ $? -ne 0 ]; then
  echo "Errors compiling test_int_laguerre_prb.f"
  exit
fi
rm compiler.txt
#
gfortran test_int_laguerre_prb.o -L$HOME/libf77/$ARCH -ltest_int_laguerre
if [ $? -ne 0 ]; then
